...
/*
	...
	pfnHook: a function pointer (NWNCXHOOK) which is called when there are no hooks.
...
*/
int SetHookDefaultForHookableEvent(HANDLE hEvent, NWNCXHOOK pfnHook);
...
*/
HANDLE HookEvent(const char *name,NWNCXHOOK hookProc);
...
*/
HANDLE CreateServiceFunction(const char *name,NWNCXSERVICE serviceProc);
